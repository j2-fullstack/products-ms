...
export class ProductsService extends PrismaClient implements OnModuleInit {
  ...
  async validateProducts(ids: number[]) {
    //Eliminamos los ids duplicados, Set() crea un set de datos sin duplicados
    ids = Array.from(new Set(ids));
    const products = await this.product.findMany({
      where: { id: { in: ids } },
    });
    //la longitud del arreglo ids que viene de la request tiene que ser de la misma longitud del array que se obtiene como resultado
